...
?>
        ...
        <main>
            ...
            <?php             
                ...
                $sql = "SELECT a.*, c.name as zh1, d.name as zh2, e.name as zh3, YEAR(a.mdate) as year FROM mnames a join mzhanret b on (a.id = b.mid) join zhanret c on (b.zh1 = c.id) join zhanret d on (b.zh2 = d.id) join zhanret e on (b.zh3 = e.id) WHERE category = 'movie' order by(year) DESC";
                $result = $db->query($sql);
                $i = 0; 
                $num = $result->num_rows;
                while($i < $num):
